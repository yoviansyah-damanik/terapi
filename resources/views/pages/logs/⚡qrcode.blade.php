...
new #[Layout('layouts::app')] #[Title('Log QR Code')] class extends Component {
    use WithPagination;
    ...
    {
        $this->viewLogId = $id;
        $this->dispatch('open-qrcode-log-detail');
    }
    ...
    {
        $query = ApiLog::forScope('qrcode')->when($this->search, fn($q) => $q->search($this->search))->when($this->filterStatus, fn($q) => $q->forStatus($this->filterStatus))->when($this->filterDate, fn($q) => $q->whereDate('created_at', $this->filterDate))->orderByDesc('created_at');

        $stats = ApiLog::forScope('qrcode')
            ->selectRaw(
                "
            COUNT(*) as total,
            SUM(response_status BETWEEN 200 AND 299) as success,
            SUM(response_status NOT BETWEEN 200 AND 299) as failed,
...
<div @open-qrcode-log-detail.window="$flux.modal('qrcode-log-detail').show()">
    <x-ui.page-header title="Log QR Code" subtitle="Riwayat aktivitas generate QR Code via API">
        <x-slot:actions>
    ...
    <div class="grid grid-cols-2 gap-3 mb-5 sm:grid-cols-5">
        <x-organisms.stat-card title="Total" value="{{ number_format($stats->total ?? 0) }}" icon="queue-list"
            color="zinc" />
        <x-organisms.stat-card title="Sukses" value="{{ number_format($stats->success ?? 0) }}" icon="check-circle"
            color="emerald" />
        <x-organisms.stat-card title="Gagal" value="{{ number_format($stats->failed ?? 0) }}" icon="x-circle"
            color="red" />
        <x-organisms.stat-card title="Hari Ini" value="{{ number_format($stats->today ?? 0) }}" icon="calendar-days"
            color="blue" />
        <x-organisms.stat-card title="Avg. Resp" value="{{ number_format($stats->avg_ms ?? 0) }} ms" icon="clock"
            color="violet" />
    </div>

    {{-- Data Panel --}}
    <x-organisms.data-panel title="Log Aktivitas QR Code" subtitle="Riwayat request API generate QR Code">
        <x-slot:filter>
            <div class="flex flex-wrap gap-3">
                <flux:input wire:model.live.debounce.300ms="search" placeholder="Cari user API, IP…"
        ...
        <div class="overflow-x-auto">
            <x-organisms.table>
                <x-slot:headings>
                    ...
                    <x-atoms.table-heading class="hidden lg:table-cell w-36">IP</x-atoms.table-heading>
                    <x-atoms.table-heading class="hidden md:table-cell" align="center">Resp.
                        Time</x-atoms.table-heading>
                    <x-atoms.table-heading class="w-28" align="center">Status</x-atoms.table-heading>
                    ...
                    <x-molecules.table-row :key="$log->id"
                        ...
                        <x-atoms.table-cell>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-mono text-zinc-500 dark:text-primary-dark-400 truncate"
                                    title="{{ $log->path }}">{{ $log->path }}</span>
                            </div>
                        </x-atoms.table-cell>
                        <x-atoms.table-cell class="hidden md:table-cell">
                            <span
                                class="text-xs text-zinc-500 dark:text-primary-dark-400">{{ $log->api_user_name ?? '—' }}</span>
                        </x-atoms.table-cell>
                        <x-atoms.table-cell class="hidden lg:table-cell">
                            <span
                                class="font-mono text-xs text-zinc-500 dark:text-primary-dark-400">{{ $log->ip_address ?? '—' }}</span>
                        </x-atoms.table-cell>
                        ...
                        <x-atoms.table-cell colspan="7" class="py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-sm text-zinc-400 dark:text-primary-dark-500">
                                    {{ $search || $filterStatus || $filterDate ? 'Tidak ada log yang sesuai filter.' : 'Belum ada log QR Code.' }}
                                </p>
                            </div>
                        </x-atoms.table-cell>
                    </x-molecules.table-row>
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal name="qrcode-log-detail" @close="$wire.set('viewLogId', null)" maxWidth="3xl" title="">
        @if ($viewLog)
            @php
                $rawTabs = [
                    'response' => [
                ...
                <div class="flex items-start gap-3 pb-4 border-b border-zinc-200 dark:border-primary-dark-700">
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            <span
                                class="font-mono text-sm font-semibold text-zinc-700 dark:text-primary-dark-300 truncate">
                                {{ $viewLog->path }}</span>
                        </div>
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    @php
                        $infoRows = array_filter(
                            [
                                ...
                                [
                                    'label' => 'Response Time',
                                    'value' => $viewLog->response_time_ms
                                        ? number_format($viewLog->response_time_ms) . ' ms'
                                        : null,
                                ],
                            ],
                    ...
                    @foreach ($infoRows as $row)
                        <div class="flex flex-col gap-0.5 p-3 rounded-xl bg-zinc-50 dark:bg-primary-dark-900/50">
                            <span
                                class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-primary-dark-500">{{ $row['label'] }}</span>
                            <span
                                class="text-sm font-medium text-zinc-700 dark:text-primary-dark-300 font-mono break-all">{{ $row['value'] }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                @foreach ($tabs as $tabKey => $tab)
                    <div x-show="tab === '{{ $tabKey }}'">
                        <x-atoms.code-block language="json"
                            maxHeight="max-h-64">{{ json_encode($tab['data'], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}</x-atoms.code-block>
                    </div>
                @endforeach

                @if (empty($tabs))
                    <p class="text-sm text-center text-zinc-400 dark:text-primary-dark-500 py-6">Tidak ada payload
                        untuk log ini.</p>
                @endif

                <div class="flex justify-end pt-2">
                    <x-atoms.button size="sm" variant="ghost" icon="x-mark"
                        x-on:click="$flux.modal('qrcode-log-detail').close()">Tutup</x-atoms.button>
                </div>
            </div>
        @endif
    </x-organisms.modal>
