            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-primary-dark-700">
                <thead class="bg-zinc-50 dark:bg-primary-dark-900">
                    <tr>
                        ...
                        <th class="{{ $thClass }} text-left">Dosis</th>
                        <th class="{{ $thClass }} text-right">Jumlah</th>
                        <th class="{{ $thClass }} text-right hidden md:table-cell">Harga</th>
                    ...
                    @foreach ($items as $index => $rp)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-primary-dark-700/50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="text-sm text-zinc-900 dark:text-primary-dark-100">{{ $rp->tanggal?->format('d/m/Y') }}</div>
                                <div class="text-xs text-zinc-500 dark:text-primary-dark-400">{{ $rp->jam }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-sm font-medium text-zinc-900 dark:text-primary-dark-100">
                                    {{ $rp->dataBarang?->nm_brng ?? $rp->kode_brng }}
                                </p>
                                <p class="text-xs text-zinc-500 dark:text-primary-dark-400">{{ $rp->kode_brng }}</p>
                                @if ($rp->no_batch)
                                    @php $batch = $rp->dataBatch; @endphp
                                    <div class="mt-1 flex flex-wrap gap-x-3 text-[10px] text-zinc-400 dark:text-primary-dark-500">
                                        <span>Batch: <span class="font-medium text-zinc-500 dark:text-primary-dark-400">{{ $rp->no_batch }}</span></span>
                ...
                <tfoot class="bg-zinc-50 dark:bg-primary-dark-900">
                    <tr>
                        <td colspan="6" class="px-4 py-3 text-sm font-semibold text-right text-zinc-900 dark:text-primary-dark-100">
                            Total {{ $title }}
                        </td>
