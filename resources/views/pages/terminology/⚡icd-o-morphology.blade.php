...
new #[Layout('layouts::app', ['title' => 'Source Terminology'])] class extends Component {
    ...
    protected string $type = 'icd_o_morphology';
    protected string $typeLabel = 'ICD-O Morphology';
    ...
    public function previewImport(): void
    {
        ...
        foreach ($this->importVersions as $v) {
            $count = IcdOMorphology::countByVersion($v);
            if ($count > 0) {
    ...
    {
        IcdOMorphology::where('version', $version)->delete();
    ...
    {
        $data = IcdOMorphology::query()->when($this->search, fn($q) => $q->where(fn($s) => $s->where('code', 'like', $this->search . '%')->orWhere('display', 'like', '%' . $this->search . '%')))->when($this->filterVersion, fn($q) => $q->where('version', $this->filterVersion))->orderBy('version')->orderBy('code')->paginate(25);
