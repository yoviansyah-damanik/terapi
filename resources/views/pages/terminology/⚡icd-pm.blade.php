...
new #[Layout('layouts::app', ['title' => 'ICD-PM'])] class extends Component {
    use WithPagination;

    public string $search = '';
    public string $filterVersion = '';
    public string $filterCategory = '';
    public string $filterSubcat = '';
    ...
    public function updatingFilterSubcat(): void
    {
        $this->resetPage();
    }
    ...
    public function previewImport(): void
    {
        ...
        // ICD-PM butuh kolom: code, display, version (tanpa level)
        ...
        if (in_array('level', $headers)) {
            fclose($handle);
            $this->addError('importPath', 'Format tidak sesuai ICD-PM. Kolom LEVEL tidak boleh ada (gunakan file ICD-MM untuk format tersebut).');
            return;
        }
    ...
    private function doDispatch(array $forceVersions): void
    {
        ...
        $this->importing = true;
        try {
            ImportIcdPmJob::dispatch($this->importPath, $forceVersions);
            $this->resetImportState();
    ...
    {
        IcdPm::where('version', $version)->delete();
    ...
    {
        $data = IcdPm::when($this->search, fn($q) => $q->where(fn($s) => $s->where('code', 'like', $this->search . '%')->orWhere('display', 'like', '%' . $this->search . '%')))->when($this->filterVersion, fn($q) => $q->where('version', $this->filterVersion))->when($this->filterCategory, fn($q) => $q->where('category', $this->filterCategory))->when($this->filterSubcat, fn($q) => $q->where('subcategory', $this->filterSubcat))->orderBy('version')->orderBy('category')->orderBy('subcategory')->orderBy('code')->paginate(25);

        return [
            'data' => $data,
            'versions' => IcdPm::getVersions(),
            'categories' => IcdPm::getCategories($this->filterVersion),
            'subcategories' => IcdPm::getSubcategories($this->filterVersion, $this->filterCategory),
        ];
    }
};
?>
...
<div>
    <x-organisms.data-panel title="Source Terminology — ICD-PM" subtitle="International Classification of Diseases for Perinatal Mortality">
        <x-slot:actions>
        ...
        <x-slot:filter>
            <div class="flex flex-wrap items-end gap-3 mb-4">
                ...
                <div class="min-w-52">
                    <flux:select wire:model.live="filterCategory" size="sm">
                        <option value="">Semua Kategori</option>
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->category }}">{{ $cat->category }} — {{ $cat->category_display }}</option>
                        @endforeach
                    </flux:select>
                </div>
                ...
                @if ($filterCategory)
                    <div class="min-w-52">
                        <flux:select wire:model.live="filterSubcat" size="sm">
                            <option value="">Semua Subkategori</option>
                            @foreach ($subcategories as $sub)
                                <option value="{{ $sub->subcategory }}">{{ $sub->subcategory }} —
                                    {{ Str::limit($sub->subcategory_display, 40) }}</option>
                            @endforeach
                        </flux:select>
                    </div>
                @endif
        
                @if ($search || $filterVersion || $filterCategory || $filterSubcat)
                    <x-atoms.button
                        wire:click="$set('search', ''); $set('filterVersion', ''); $set('filterCategory', ''); $set('filterSubcat', '')"
                ...
                @if ($filterVersion)
                    <x-atoms.button wire:click="deleteVersion('{{ $filterVersion }}')"
                        wire:confirm="Hapus semua data versi {{ $filterVersion }} ICD-PM?" icon="trash" variant="ghost"
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <x-atoms.table-heading class="w-40">Kategori</x-atoms.table-heading>
                <x-atoms.table-heading class="w-44">Subkategori</x-atoms.table-heading>
            ...
            @forelse ($data as $row)
                <x-molecules.table-row :key="$row->id">
                    ...
                    <x-atoms.table-cell>
                        @if ($row->category)
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <flux:badge color="purple" size="sm">{{ $row->category }}</flux:badge>
                                <span class="text-xs text-zinc-500 dark:text-primary-dark-400">{{ $row->category_display }}</span>
                            </div>
                        @else
                    ...
                    <x-atoms.table-cell>
                        @if ($row->subcategory)
                            <span class="font-mono font-medium text-zinc-700 dark:text-primary-dark-300">{{ $row->subcategory }}</span>
                            @if ($row->subcategory_display)
                                <br><span class="text-[11px] text-zinc-400 dark:text-primary-dark-500">{{ Str::limit($row->subcategory_display, 38) }}</span>
                            @endif
                        @else
                ...
                <x-molecules.table-row>
                    <x-atoms.table-cell colspan="5" class="py-12 text-center">
                        <div class="flex flex-col items-center gap-2 text-zinc-400 dark:text-primary-dark-600">
                            ...
                            <div class="text-sm">Belum ada data ICD-PM</div>
    ...
    {{-- ===================== Modal Import ===================== --}}
    <x-organisms.modal wire:model="showImportModal" maxWidth="md" title="Import CSV — ICD-PM">
