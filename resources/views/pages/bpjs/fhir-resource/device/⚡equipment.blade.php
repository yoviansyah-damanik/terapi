...
new #[Layout('layouts::app')] #[Title('BPJS — Device (Alat Kesehatan)')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsDevice $selectedItem = null;
    ...
    {
        if (BpjsDevice::where('local_code', $localCode)->exists()) {
            $this->toastWarning('Alat kesehatan ini sudah memiliki UUID BPJS.');
            return;
        }

        BpjsDevice::create(['local_code' => $localCode, 'name' => $name]);
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua Alat Kesehatan dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $localCode): void
    {
        $this->selectedItem = BpjsDevice::where('local_code', $localCode)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsDevice::where('local_code', $this->deleteCode)->delete();
        $this->showDeleteModal = false;
        $this->reset(['deleteCode', 'deleteName']);
        $this->toastSuccess('UUID BPJS Alat Kesehatan berhasil dihapus.');
    }

    public function with(): array
    {
        $registered = BpjsDevice::pluck('id', 'local_code');
        $totalRegistered = BpjsDevice::count();
        ...
        try {
            $query = InventarisBarang::where('id_jenis', 'ALKES');
            ...
            if ($this->filterStatus === 'registered') {
                $query->whereIn('kode_barang', $registered->keys()->toArray());
            } elseif ($this->filterStatus === 'unregistered') {
                $query->whereNotIn('kode_barang', $registered->keys()->toArray());
            }

            $total = InventarisBarang::where('id_jenis', 'ALKES')->count();
            $items = $query->orderBy('nama_barang')->paginate(25);
        } catch (\Exception) {
            $simrsError = true;
        }
...
<div>
    <x-ui.page-header title="BPJS — Device (Alat Kesehatan)" subtitle="Registry UUID FHIR Device untuk alat kesehatan BPJS">
        <x-slot name="actions">
    ...
    <x-organisms.data-panel :padding="false">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
                    <div class="flex-1 max-w-sm">
                        <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                            placeholder="Cari kode atau nama alat kesehatan..." clearable />
                    </div>
        ...
        <x-organisms.table>
            <x-slot:headings>
                <th class="px-5 py-3.5 text-xs font-semibold tracking-wider text-left uppercase text-zinc-400 dark:text-primary-dark-500 w-36">Kode</th>
                <th class="px-5 py-3.5 text-xs font-semibold tracking-wider text-left uppercase text-zinc-400 dark:text-primary-dark-500">Nama Alat Kesehatan</th>
            ...
            @forelse ($items as $item)
                @php $uuid = $registered[$item->kode_barang] ?? null; @endphp
                <x-molecules.table-row wire:key="dev-{{ $item->kode_barang }}">
                    ...
                    <x-atoms.table-cell>
                        <p class="text-sm font-medium text-zinc-800 dark:text-primary-dark-100">{{ $item->nama_barang }}</p>
                    </x-atoms.table-cell>
                    <x-atoms.table-cell>
                        @if ($uuid)
                    ...
                    <x-atoms.table-cell align="center" action>
                        @if (!$uuid)
                            <x-atoms.button
                                wire:click="generateUuid('{{ addslashes($item->kode_barang) }}', '{{ addslashes($item->nama_barang) }}')"
                                wire:target="generateUuid('{{ addslashes($item->kode_barang) }}', '{{ addslashes($item->nama_barang) }}')"
                                size="sm" variant="primary" icon="plus-circle">
                                Generate
                            </x-atoms.button>
                        @else
                            <x-atoms.button variant="ghost" wire:click="viewDetail('{{ addslashes($item->kode_barang) }}')" size="sm" icon="eye" tooltip="Lihat detail" />
                            <x-atoms.button variant="ghost" wire:click="confirmDelete('{{ addslashes($item->kode_barang) }}', '{{ addslashes($item->nama_barang) }}')" size="sm" icon="trash" tooltip="Hapus UUID" />
                        @endif
                    </x-atoms.table-cell>
                    ...
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-primary-dark-700">
                                <flux:icon name="wrench" class="w-7 h-7 text-zinc-300 dark:text-primary-dark-500" />
                            </div>
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">
                                {{ $simrsError ? 'Koneksi SIMRS gagal' : 'Tidak ada data alat kesehatan ditemukan' }}
                            </p>
                        </div>
                    </td>
                </tr>
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua Alat Kesehatan" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} alat kesehatan belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
            <div class="p-3.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed italic">
                ...
                UUID baru akan di-generate untuk semua alat kesehatan yang belum terdaftar. Proses berjalan di background (queue worker).
            </div>
        </div>
        <x-slot name="footer">
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail Device BPJS" maxWidth="lg">
        @if ($selectedItem)
            <x-slot name="description">
                <div class="flex items-center gap-2 mt-1">
                    <span class="font-mono text-sm font-bold text-zinc-500 dark:text-primary-dark-400">{{ $selectedItem->local_code }}</span>
                    <flux:badge color="green" size="sm">Terdaftar</flux:badge>
                </div>
            </x-slot>
            <div class="space-y-6">
                ...
                <div class="pt-5 border-t border-zinc-100 dark:border-primary-dark-700/60">
                    ...
                    <dl class="space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <dt class="text-[10px] uppercase font-bold text-zinc-400 dark:text-primary-dark-500 mb-1">Local Code</dt>
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Alat Kesehatan?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
