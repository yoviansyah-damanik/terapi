...
new #[Layout('layouts::app')] #[Title('BPJS — Allergy Reaction')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsAllergyReaction $selectedItem = null;
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua Reaksi Alergi dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $localCode): void
    {
        $this->selectedItem = BpjsAllergyReaction::where('local_code', $localCode)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsAllergyReaction::where('local_code', $this->deleteCode)->delete();
    ...
    {
        $registered = BpjsAllergyReaction::pluck('id', 'local_code');
        $totalRegistered = BpjsAllergyReaction::count();
        ...
        try {
            $query = AlergiReaksi::query();

            if ($this->search) {
                $query->where(fn($q) => $q
                    ->where('id', 'like', "%{$this->search}%")
                    ->orWhere('nama_reaksi', 'like', "%{$this->search}%")
                );
            }
            ...
            $total = AlergiReaksi::count();
            $items = $query->orderBy('nama_reaksi')->paginate(25);
        } catch (\Exception) {
            $simrsError = true;
        }
...
<div>
    <x-ui.page-header title="BPJS — Allergy Reaction" subtitle="Registry UUID FHIR AllergyIntolerance untuk reaksi alergi BPJS">
        <x-slot name="actions">
    ...
    <x-organisms.data-panel :padding="false">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
                    <div class="flex-1 max-w-sm">
                        <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                            placeholder="Cari ID atau nama reaksi alergi..." clearable />
                    </div>
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <th class="px-5 py-3.5 text-xs font-semibold tracking-wider text-left uppercase text-zinc-400 dark:text-primary-dark-500">Nama Reaksi Alergi</th>
            ...
            @forelse ($items as $item)
                ...
                <x-molecules.table-row wire:key="rea-{{ $item->id }}">
                    ...
                    <x-atoms.table-cell align="center" action>
                        @if (!$uuid)
                            <x-atoms.button
                                wire:click="generateUuid('{{ $item->id }}', '{{ addslashes($item->nama_reaksi) }}')"
                                wire:target="generateUuid('{{ $item->id }}', '{{ addslashes($item->nama_reaksi) }}')"
                        ...
                        @else
                            ...
                            <x-atoms.button variant="ghost" wire:click="confirmDelete('{{ $item->id }}', '{{ addslashes($item->nama_reaksi) }}')" size="sm" icon="trash" tooltip="Hapus UUID" />
                        @endif
                    </x-atoms.table-cell>
                    ...
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-primary-dark-700">
                                <flux:icon name="fire" class="w-7 h-7 text-zinc-300 dark:text-primary-dark-500" />
                            </div>
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">
                                {{ $simrsError ? 'Koneksi SIMRS gagal' : 'Tidak ada data reaksi alergi ditemukan' }}
                            </p>
                        </div>
                    </td>
                </tr>
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua Reaksi Alergi" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} reaksi alergi belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
            <div class="p-3.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed italic">
                ...
                UUID baru akan di-generate untuk semua reaksi alergi yang belum terdaftar. Proses berjalan di background (queue worker).
            </div>
        </div>
        <x-slot name="footer">
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail Allergy Reaction BPJS" maxWidth="lg">
        @if ($selectedItem)
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Reaksi Alergi?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
