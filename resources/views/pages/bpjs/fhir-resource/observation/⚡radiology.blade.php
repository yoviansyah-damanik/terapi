...
new #[Layout('layouts::app')] #[Title('BPJS — Observation Radiologi')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsObservationRadiology $selectedItem = null;
    ...
    {
        if (BpjsObservationRadiology::where('local_code', $localCode)->exists()) {
            $this->toastWarning('Pemeriksaan radiologi ini sudah memiliki UUID BPJS.');
            return;
        }

        BpjsObservationRadiology::create(['local_code' => $localCode, 'name' => $name]);
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua Observation Radiologi dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $localCode): void
    {
        $this->selectedItem = BpjsObservationRadiology::where('local_code', $localCode)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsObservationRadiology::where('local_code', $this->deleteCode)->delete();
        $this->showDeleteModal = false;
        $this->reset(['deleteCode', 'deleteName']);
        $this->toastSuccess('UUID BPJS Observation Radiologi berhasil dihapus.');
    }

    public function with(): array
    {
        $registered = BpjsObservationRadiology::pluck('id', 'local_code');
        $totalRegistered = BpjsObservationRadiology::count();
        ...
        try {
            $query = JnsPerawatanRadiologi::query();

            if ($this->search) {
                $query->where(fn($q) => $q
                    ->where('kd_jenis_prw', 'like', "%{$this->search}%")
                    ->orWhere('nm_perawatan', 'like', "%{$this->search}%")
                );
            }

            if ($this->filterStatus === 'registered') {
                $query->whereIn('kd_jenis_prw', $registered->keys()->toArray());
            } elseif ($this->filterStatus === 'unregistered') {
                $query->whereNotIn('kd_jenis_prw', $registered->keys()->toArray());
            }

            $total = JnsPerawatanRadiologi::count();
...
<div>
    <x-ui.page-header title="BPJS — Observation Radiologi" subtitle="Registry UUID FHIR Observation untuk pemeriksaan radiologi BPJS">
        <x-slot name="actions">
    ...
    <x-organisms.data-panel :padding="false">
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <th class="px-5 py-3.5 text-xs font-semibold tracking-wider text-left uppercase text-zinc-400 dark:text-primary-dark-500">Nama Pemeriksaan</th>
            ...
            @forelse ($items as $item)
                @php $uuid = $registered[$item->kd_jenis_prw] ?? null; @endphp
                <x-molecules.table-row wire:key="rad-{{ $item->kd_jenis_prw }}">
                    ...
                    <x-atoms.table-cell>
                        <p class="text-sm font-medium text-zinc-800 dark:text-primary-dark-100">{{ $item->nm_perawatan }}</p>
                    </x-atoms.table-cell>
                    <x-atoms.table-cell>
                        @if ($uuid)
                    ...
                    <x-atoms.table-cell align="center" action>
                        @if (!$uuid)
                            <x-atoms.button
                                wire:click="generateUuid('{{ addslashes($item->kd_jenis_prw) }}', '{{ addslashes($item->nm_perawatan) }}')"
                                wire:target="generateUuid('{{ addslashes($item->kd_jenis_prw) }}', '{{ addslashes($item->nm_perawatan) }}')"
                                size="sm" variant="primary" icon="plus-circle">
                                Generate
                            </x-atoms.button>
                        @else
                            <x-atoms.button variant="ghost" wire:click="viewDetail('{{ addslashes($item->kd_jenis_prw) }}')" size="sm" icon="eye" tooltip="Lihat detail" />
                            <x-atoms.button variant="ghost" wire:click="confirmDelete('{{ addslashes($item->kd_jenis_prw) }}', '{{ addslashes($item->nm_perawatan) }}')" size="sm" icon="trash" tooltip="Hapus UUID" />
                        @endif
                    </x-atoms.table-cell>
                    ...
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">
                                {{ $simrsError ? 'Koneksi SIMRS gagal' : 'Tidak ada data pemeriksaan radiologi ditemukan' }}
                            </p>
                        </div>
                    </td>
                </tr>
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua Observation Radiologi" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} pemeriksaan radiologi belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
            <div class="p-3.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed italic">
                ...
                UUID baru akan di-generate untuk semua pemeriksaan yang belum terdaftar. Proses berjalan di background (queue worker).
            </div>
        </div>
        <x-slot name="footer">
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail Observation Radiologi BPJS" maxWidth="lg">
        @if ($selectedItem)
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Observation Radiologi?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
