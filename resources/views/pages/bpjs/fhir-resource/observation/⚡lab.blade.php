...
new #[Layout('layouts::app')] #[Title('BPJS — Observation Lab')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsObservationLab $selectedItem = null;
    ...
    {
        if (BpjsObservationLab::where('local_code', $localCode)->exists()) {
            $this->toastWarning('Pemeriksaan lab ini sudah memiliki UUID BPJS.');
            return;
        }

        BpjsObservationLab::create(['local_code' => $localCode, 'name' => $name]);
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua Observation Lab dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $localCode): void
    {
        $this->selectedItem = BpjsObservationLab::where('local_code', $localCode)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsObservationLab::where('local_code', $this->deleteCode)->delete();
        $this->showDeleteModal = false;
        $this->reset(['deleteCode', 'deleteName']);
        $this->toastSuccess('UUID BPJS Observation Lab berhasil dihapus.');
    }

    public function with(): array
    {
        $registered = BpjsObservationLab::pluck('id', 'local_code');
        $totalRegistered = BpjsObservationLab::count();
...
<div>
    <x-ui.page-header title="BPJS — Observation Lab" subtitle="Registry UUID FHIR Observation untuk pemeriksaan laboratorium BPJS">
        <x-slot name="actions">
                    ...
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">
                                {{ $simrsError ? 'Koneksi SIMRS gagal' : 'Tidak ada data pemeriksaan lab ditemukan' }}
                            </p>
                        </div>
                    </td>
                </tr>
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua Observation Lab" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} pemeriksaan lab belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail Observation Lab BPJS" maxWidth="lg">
        @if ($selectedItem)
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Observation Lab?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
