...
new #[Layout('layouts::app')] #[Title('BPJS — ICD-9CM')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsIcd9 $selectedIcd = null;
    ...
    public function generateUuid(string $code, string $display): void
    {
        if (BpjsIcd9::where('code', $code)->exists()) {
            $this->toastWarning('Kode ICD-9CM ini sudah memiliki UUID BPJS.');
            return;
        }
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua ICD-9CM dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $code): void
    {
        $this->selectedIcd = BpjsIcd9::where('code', $code)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsIcd9::where('code', $this->deleteCode)->delete();
        $this->showDeleteModal = false;
        $this->reset(['deleteCode', 'deleteName']);
        $this->toastSuccess('UUID BPJS ICD-9CM berhasil dihapus.');
    }

    public function with(): array
    {
        $registered = BpjsIcd9::pluck('id', 'code');

        $query = Icd9::query()->select('code', \DB::raw('MAX(display) as display'));
        ...
        $totalRegistered = BpjsIcd9::count();
...
<div>
    <x-ui.page-header title="BPJS — ICD-9CM" subtitle="Registry UUID FHIR Procedure untuk kode tindakan ICD-9CM BPJS">
        <x-slot name="actions">
    ...
    <x-organisms.data-panel :padding="false">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
                    <div class="flex-1 max-w-sm">
                        <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                            placeholder="Cari kode atau nama tindakan..." clearable />
                    </div>
        ...
        <x-organisms.table>
            ...
            @forelse ($records as $icd)
                @php $uuid = $registered[$icd->code] ?? null; @endphp
                <x-molecules.table-row wire:key="icd9-{{ $icd->code }}">
                    <x-atoms.table-cell>
                        <span
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua ICD-9CM" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} kode ICD-9CM belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
            <div
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail ICD-9CM BPJS" maxWidth="lg">
        @if ($selectedIcd)
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Tindakan?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                <div
