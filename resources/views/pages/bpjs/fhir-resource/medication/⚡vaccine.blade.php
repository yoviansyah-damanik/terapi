...
new #[Layout('layouts::app')] #[Title('BPJS — Medication Vaccine')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsVaccine $selectedItem = null;
    ...
    {
        if (BpjsVaccine::where('local_code', $localCode)->exists()) {
            $this->toastWarning('Vaksin ini sudah memiliki UUID BPJS.');
            return;
        }

        BpjsVaccine::create(['local_code' => $localCode, 'name' => $name]);
    ...
    public function syncAll(): void
    {
        ...
        $this->toastSuccess('Sync semua Vaksin dijadwalkan. Proses berjalan di background.');
    }

    public function viewDetail(string $localCode): void
    {
        $this->selectedItem = BpjsVaccine::where('local_code', $localCode)->first();
    ...
    public function deleteEntry(): void
    {
        ...
        BpjsVaccine::where('local_code', $this->deleteCode)->delete();
        $this->showDeleteModal = false;
        $this->reset(['deleteCode', 'deleteName']);
        $this->toastSuccess('UUID BPJS Vaksin berhasil dihapus.');
    }

    public function with(): array
    {
        $registered = BpjsVaccine::pluck('id', 'local_code');
        $totalRegistered = BpjsVaccine::count();
        ...
        try {
            $query = DataBarang::where('nama_brng', 'like', 'vaksin%');

            if ($this->search) {
                $query->where(fn($q) => $q
                    ->where('kode_brng', 'like', "%{$this->search}%")
                    ->orWhere('nama_brng', 'like', "%{$this->search}%")
                );
            }
            ...
            $total = DataBarang::where('nama_brng', 'like', 'vaksin%')->count();
...
<div>
    <x-ui.page-header title="BPJS — Medication Vaccine" subtitle="Registry UUID FHIR Immunization untuk vaksin BPJS">
        <x-slot name="actions">
    ...
    <x-organisms.data-panel :padding="false">
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <th class="px-5 py-3.5 text-xs font-semibold tracking-wider text-left uppercase text-zinc-400 dark:text-primary-dark-500">Nama Vaksin</th>
            ...
            @forelse ($items as $item)
                ...
                <x-molecules.table-row wire:key="vac-{{ $item->kode_brng }}">
                    ...
                    <x-atoms.table-cell>
                        <p class="text-sm font-medium text-zinc-800 dark:text-primary-dark-100">{{ $item->nama_brng }}</p>
                    </x-atoms.table-cell>
                    <x-atoms.table-cell>
                        @if ($uuid)
                    ...
                    <x-atoms.table-cell align="center" action>
                        ...
                        @else
                            <x-atoms.button variant="ghost" wire:click="viewDetail('{{ addslashes($item->kode_brng) }}')" size="sm" icon="eye" tooltip="Lihat detail" />
                            <x-atoms.button variant="ghost" wire:click="confirmDelete('{{ addslashes($item->kode_brng) }}', '{{ addslashes($item->nama_brng) }}')" size="sm" icon="trash" tooltip="Hapus UUID" />
                        @endif
                    </x-atoms.table-cell>
                    ...
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-primary-dark-700">
                                <flux:icon name="shield-check" class="w-7 h-7 text-zinc-300 dark:text-primary-dark-500" />
                            </div>
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">
                                {{ $simrsError ? 'Koneksi SIMRS gagal' : 'Tidak ada data vaksin ditemukan' }}
                            </p>
                        </div>
                    </td>
                </tr>
    ...
    {{-- Modal Sync Semua --}}
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua Vaksin" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                        {{ number_format($unsyncedCount) }} vaksin belum memiliki UUID BPJS
                    </p>
                </div>
            </div>
            <div class="p-3.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed italic">
                ...
                UUID baru akan di-generate untuk semua vaksin yang belum terdaftar. Proses berjalan di background (queue worker).
            </div>
        </div>
        <x-slot name="footer">
    ...
    {{-- Modal Detail --}}
    <x-organisms.modal wire:model="showDetailModal" title="Detail Vaccine BPJS" maxWidth="lg">
        @if ($selectedItem)
    ...
    <x-organisms.modal wire:model="showDeleteModal" title="Hapus UUID Vaksin?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
