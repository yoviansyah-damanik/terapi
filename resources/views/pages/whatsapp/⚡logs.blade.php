...
new #[Layout('layouts::app')] #[Title('Log WhatsApp')] class extends Component {
    ...
    public function with(): array
    {
        ...
        $model = $isWaha ? WahaLog::class : GowaLog::class;

        $query = $model::query()->when($this->search, fn($q) => $q->where('phone', 'like', "%{$this->search}%"))->when($this->filterDirection, fn($q) => $q->where('direction', $this->filterDirection))->when($this->filterStatus, fn($q) => $q->where('status', $this->filterStatus))->orderByDesc('created_at');
...
<div>
    {{-- Header --}}
    <x-ui.page-header title="Log WhatsApp" subtitle="Riwayat request/response API WhatsApp">
        <x-slot name="actions">
