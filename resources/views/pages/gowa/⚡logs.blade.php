...
new #[Layout('layouts::app')] #[Title('Log GOWA')] class extends Component {
    use WithPagination;
    ...
    {
        $this->detailLog = GowaLog::findOrFail($id);
        $this->showDetailModal = true;
    }

    public function with(): array
    {
        $query = GowaLog::query()->when($this->search, fn($q) => $q->where('phone', 'like', "%{$this->search}%"))->when($this->filterDirection, fn($q) => $q->where('direction', $this->filterDirection))->when($this->filterStatus, fn($q) => $q->where('status', $this->filterStatus))->orderByDesc('created_at');
...
<div>
    {{-- Header --}}
    <x-ui.page-header title="Log GOWA" subtitle="Riwayat request/response API GOWA Gateway" />
