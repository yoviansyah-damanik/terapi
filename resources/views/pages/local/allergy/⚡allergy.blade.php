...
new #[Layout('layouts::app')] #[Title('Alergi — Mapping & UUID')] class extends Component {
    use WithPagination;
    ...
    public ?BpjsAllergy $selectedBpjsItem = null;
    ...
    public function openSnomedModal(int $id, string $name): void
    {
        ...
        $this->snomedInitialSearch = AllergyMap::where('allergy_id', $id)->value('system_term') ?? '';
    ...
    {
        AllergyMap::where('allergy_id', $this->deleteSnomedId)->delete();
        AllergyMap::clearCache();
    ...
    {
        $this->selectedBpjsItem = BpjsAllergy::where('local_code', $localCode)->first();
    ...
    {
        BpjsAllergy::where('local_code', $this->deleteBpjsCode)->delete();
    ...
    public function with(): array
    {
        try {
            $items = Alergi::query()
                ->when($this->search, fn($q) => $q->where('nama_alergi', 'like', "%{$this->search}%")->orWhere('tipe', 'like', "%{$this->search}%"))
                ->when($this->filterType, fn($q) => $q->where('tipe', $this->filterType))
                ->orderBy('nama_alergi')
                ->paginate($this->perPage);

            $types = Alergi::query()->whereNotNull('tipe')->distinct()->orderBy('tipe')->pluck('tipe')->toArray();
        } catch (\Exception) {
        ...
        $mappings = AllergyMap::whereIn('allergy_id', $ids)->get()->keyBy('allergy_id');

        $bpjsRegistered = BpjsAllergy::whereIn('local_code', array_map('strval', $ids))
            ->pluck('id', 'local_code');
        ...
        $totalBpjs   = BpjsAllergy::count();
        $totalSimrs  = Alergi::count();
        $totalSnomed = AllergyMap::count();
...
<div>
    <x-ui.page-header title="Alergi — Mapping & UUID"
        subtitle="Kelola mapping SNOMED CT dan UUID BPJS untuk data alergi dalam satu tampilan">
        <x-slot name="actions">
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <x-organisms.stat-card title="Total Alergi" :value="number_format($totalSimrs)" icon="shield-exclamation" color="zinc" />
    ...
    <x-organisms.data-panel>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-40">
                    <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                        placeholder="Cari nama alergi atau tipe..." clearable />
                </div>
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <x-atoms.table-heading class="w-32">Tipe</x-atoms.table-heading>
            ...
            @forelse ($items as $item)
                <x-molecules.table-row wire:key="alergi-{{ $item->id }}">
                    ...
                    <x-atoms.table-cell>
                        @if ($item->tipe)
                            <flux:badge color="sky" size="sm">{{ $item->tipe }}</flux:badge>
                        @else
                    ...
                    <x-atoms.table-cell align="center" action nowrap>
                        ...
                        <div class="flex items-center gap-0.5 border-r border-zinc-200 dark:border-primary-dark-600 pr-2 mr-2">
                            <x-atoms.button
                                wire:click="openSnomedModal({{ $item->id }}, '{{ addslashes($item->nama_alergi) }}')"
                            ...
                            @if ($item->snomed_code)
                                <x-atoms.button
                                    wire:click="confirmDeleteSnomed({{ $item->id }}, '{{ addslashes($item->nama_alergi) }}')"
                        ...
                        <div class="flex items-center gap-0.5">
                            @if (!$item->bpjs_uuid)
                                <x-atoms.button
                                    wire:click="generateBpjsUuid({{ $item->id }}, '{{ addslashes($item->nama_alergi) }}')"
                            ...
                            @else
                                ...
                                <x-atoms.button variant="ghost"
                                    wire:click="confirmDeleteBpjs('{{ $item->id }}', '{{ addslashes($item->nama_alergi) }}')"
                    ...
                    <x-atoms.table-cell colspan="6" align="center" class="py-16">
                        <div class="flex flex-col items-center gap-3">
                            <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-primary-dark-700">
                                <flux:icon name="shield-exclamation" class="w-7 h-7 text-zinc-300 dark:text-primary-dark-500" />
                            </div>
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">Tidak ada data alergi</p>
        ...
        <div class="space-y-4">
            <div>
                ...
                <flux:text class="mt-0.5">
                    Untuk Alergi: <span class="font-semibold text-zinc-700 dark:text-primary-dark-200">{{ $selectedName }}</span>
                    <span class="font-mono text-xs text-zinc-400 ml-1">(ID: {{ $selectedId }})</span>
                </flux:text>
            </div>
            <livewire:components.snomed-search defaultTag="substance" :initialSearch="$snomedInitialSearch" :key="'snomed-alergi-' . ($selectedId ?? '')" />
    ...
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua UUID BPJS Alergi" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                    {{ number_format($unsyncedBpjs) }} data alergi belum memiliki UUID BPJS
                </p>
            </div>
    ...
    <x-organisms.modal wire:model="showBpjsDetailModal" title="Detail UUID BPJS Alergi" maxWidth="md">
        @if ($selectedBpjsItem)
            <dl class="space-y-4">
                <div>
                    <dt class="text-[10px] uppercase font-bold text-zinc-400 dark:text-primary-dark-500 mb-1">Nama Alergi</dt>
    ...
    <x-organisms.modal wire:model="showDeleteBpjsModal" title="Hapus UUID BPJS?" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                    UUID BPJS untuk alergi <strong class="text-zinc-800 dark:text-white">{{ $deleteBpjsName }}</strong> akan dihapus.
                </p>
            </div>
