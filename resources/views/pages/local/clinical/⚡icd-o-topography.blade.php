...
new #[Layout('layouts::app')] #[Title('Mapping ICD-O Topography → SNOMED CT')] class extends Component {
    ...
    public string $snomedSemanticTag = 'body structure';
    ...
    protected function saveSmartMapping(string $sourceCodeComposite, string $snomedCode, string $snomedTerm): bool
    {
        ...
        IcdOTopographyMap::updateOrCreate(
    ...
    {
        $items = IcdOTopography::query()
            ->select('icd_o_topography.*', 'map_icd_o_topography.system_code as snomed_code')
            ->leftJoin('map_icd_o_topography', function ($join) {
                $join->on('icd_o_topography.code', '=', 'map_icd_o_topography.code')->on('icd_o_topography.version', '=', 'map_icd_o_topography.version');
            })
            ->when($this->search, fn($q) => $q->where('icd_o_topography.code', 'like', "%{$this->search}%")->orWhere('icd_o_topography.display', 'like', "%{$this->search}%"))
            ->when($this->filterVersion, fn($q) => $q->where('icd_o_topography.version', $this->filterVersion))
            ->orderBy('icd_o_topography.version')
            ->orderBy('icd_o_topography.code')
    ...
    public function with(): array
    {
        ...
        $kunciData = 'icd_o_topo.data.' . md5("{$this->search}|{$this->filterVersion}|{$this->perPage}|{$halaman}");

        $items = Cache::remember(
            $kunciData,
            300,
            fn() => IcdOTopography::query()
                ->select('icd_o_topography.*', 'map_icd_o_topography.system_code as snomed_code', 'map_icd_o_topography.system_term  as snomed_term')
                ->leftJoin('map_icd_o_topography', function ($join) {
                    $join->on('icd_o_topography.code', '=', 'map_icd_o_topography.code')->on('icd_o_topography.version', '=', 'map_icd_o_topography.version');
                })
                ->when($this->search, fn($q) => $q->where('icd_o_topography.code', 'like', "%{$this->search}%")->orWhere('icd_o_topography.display', 'like', "%{$this->search}%"))
        ...
        return [
            'items' => $items,
            'versions' => IcdOTopography::getVersions(),
        ];
    }
    ...
    public function openModal(string $code, string $name, string $version): void
    {
        ...
        $this->snomedInitialSearch = IcdOTopographyMap::where('code', $code)->where('version', $version)->value('system_term') ?? '';
    ...
    public function snomedSelected(string $system_code, string $system_term, string $system_display, string $category): void
    {
        ...
        IcdOTopography::clearCache();
        IcdOTopographyMap::clearCache();
    ...
    {
        IcdOTopographyMap::where('code', $this->deleteCode)->where('version', $this->deleteVersion)->delete();
        IcdOTopography::clearCache();
        IcdOTopographyMap::clearCache();
...
<div>
    <x-ui.page-header title="Mapping ICD-O Topography → SNOMED CT"
        subtitle="Hubungkan kode topografi ICD-O dengan konsep SNOMED CT lokal" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-40">
                    <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                        placeholder="Cari kode atau deskripsi ICD-O Topography..." clearable />
                </div>
                @if (count($versions) > 0)
    ...
    <x-organisms.modal wire:model="showSnomedSearchModal" maxWidth="4xl" title="Pilih Kode SNOMED CT">
        <div class="space-y-4">
            ...
            <livewire:components.snomed-search defaultTag="body structure" :initialSearch="$snomedInitialSearch" :key="'snomed-icd-o-topo-' . ($selectedCode ?? '') . '-' . ($selectedVersion ?? '')" />
