...
new #[Layout('layouts::app')] #[Title('Mapping ICD-O Morphology → SNOMED CT')] class extends Component {
    ...
    public string $snomedSemanticTag = 'morphologic abnormality';
    ...
    protected function saveSmartMapping(string $sourceCodeComposite, string $snomedCode, string $snomedTerm): bool
    {
        ...
        IcdOMorphologyMap::updateOrCreate(
    ...
    {
        $items = IcdOMorphology::query()
            ->select('icd_o_morphology.*', 'map_icd_o_morphology.system_code as snomed_code')
            ->leftJoin('map_icd_o_morphology', function ($join) {
                $join->on('icd_o_morphology.code', '=', 'map_icd_o_morphology.code')->on('icd_o_morphology.version', '=', 'map_icd_o_morphology.version');
            })
            ->when($this->search, fn($q) => $q->where('icd_o_morphology.code', 'like', "%{$this->search}%")->orWhere('icd_o_morphology.display', 'like', "%{$this->search}%"))
            ->when($this->filterVersion, fn($q) => $q->where('icd_o_morphology.version', $this->filterVersion))
            ->orderBy('icd_o_morphology.version')
            ->orderBy('icd_o_morphology.code')
    ...
    public function with(): array
    {
        ...
        $kunciData = 'icd_o_morph.data.' . md5("{$this->search}|{$this->filterVersion}|{$this->perPage}|{$halaman}");

        $items = Cache::remember(
            $kunciData,
            300,
            fn() => IcdOMorphology::query()
                ->select('icd_o_morphology.*', 'map_icd_o_morphology.system_code as snomed_code', 'map_icd_o_morphology.system_term  as snomed_term')
                ->leftJoin('map_icd_o_morphology', function ($join) {
                    $join->on('icd_o_morphology.code', '=', 'map_icd_o_morphology.code')->on('icd_o_morphology.version', '=', 'map_icd_o_morphology.version');
                })
                ->when($this->search, fn($q) => $q->where('icd_o_morphology.code', 'like', "%{$this->search}%")->orWhere('icd_o_morphology.display', 'like', "%{$this->search}%"))
        ...
        return [
            'items' => $items,
            'versions' => IcdOMorphology::getVersions(),
        ];
    }
    ...
    public function openModal(string $code, string $name, string $version): void
    {
        ...
        $this->snomedInitialSearch = IcdOMorphologyMap::where('code', $code)->where('version', $version)->value('system_term') ?? '';
    ...
    public function snomedSelected(string $system_code, string $system_term, string $system_display, string $category): void
    {
        ...
        IcdOMorphology::clearCache();
        IcdOMorphologyMap::clearCache();
    ...
    {
        IcdOMorphologyMap::where('code', $this->deleteCode)->where('version', $this->deleteVersion)->delete();
        IcdOMorphology::clearCache();
        IcdOMorphologyMap::clearCache();
...
<div>
    <x-ui.page-header title="Mapping ICD-O Morphology → SNOMED CT"
        subtitle="Hubungkan kode morfologi ICD-O dengan konsep SNOMED CT lokal" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-40">
                    <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                        placeholder="Cari kode atau deskripsi ICD-O Morphology..." clearable />
                </div>
                @if (count($versions) > 0)
    ...
    <x-organisms.modal wire:model="showSnomedSearchModal" maxWidth="4xl" title="Pilih Kode SNOMED CT">
        <div class="space-y-4">
            ...
            <livewire:components.snomed-search defaultTag="morphologic abnormality" :initialSearch="$snomedInitialSearch" :key="'snomed-icd-o-morph-' . ($selectedCode ?? '') . '-' . ($selectedVersion ?? '')" />
