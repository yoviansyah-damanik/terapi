...
new #[Layout('layouts::app')] #[Title('ICD-9CM — Mapping & UUID')] class extends Component {
    ...
    public ?BpjsIcd9 $selectedBpjsIcd = null;
    ...
    {
        $items = Icd9::query()
            ->select('icd9.*', 'map_icd9.system_code as snomed_code')
            ->leftJoin('map_icd9', 'icd9.code', '=', 'map_icd9.icd9_code')
            ->when($this->search, fn($q) => $q->where('icd9.code', 'like', "%{$this->search}%")->orWhere('icd9.display', 'like', "%{$this->search}%"))
            ->when($this->filterVersion, fn($q) => $q->where('icd9.version', $this->filterVersion))
            ->orderBy('icd9.code')
    ...
    public function openSnomedModal(string $code, string $name): void
    {
        ...
        $this->snomedInitialSearch = Icd9Map::where('icd9_code', $code)->value('system_term') ?? '';
    ...
    {
        Icd9Map::where('icd9_code', $this->deleteSnomedCode)->delete();
        Cache::flush();
    ...
    {
        if (BpjsIcd9::where('code', $code)->exists()) {
            $this->toastWarning('Kode ICD-9 ini sudah memiliki UUID BPJS.');
            return;
        }

        BpjsIcd9::create(['code' => $code, 'display' => $display]);
    ...
    {
        $this->selectedBpjsIcd = BpjsIcd9::where('code', $code)->first();
    ...
    {
        BpjsIcd9::where('code', $this->deleteBpjsCode)->delete();
        $this->showDeleteBpjsModal = false;
        $this->reset(['deleteBpjsCode', 'deleteBpjsName']);
        $this->toastSuccess('UUID BPJS ICD-9 berhasil dihapus.');
    }
    ...
    public function with(): array
    {
        ...
        $kunciData = 'icd9.merged.' . md5("{$this->search}|{$this->filterVersion}|{$this->filterStatus}|{$this->perPage}|{$halaman}");

        $bpjsRegistered = BpjsIcd9::pluck('id', 'code');

        $items = Cache::remember($kunciData, 300, function () use ($bpjsRegistered) {
            return Icd9::query()
                ->select(
                    'icd9.code',
                    \DB::raw('MAX(icd9.display) as display'),
                    \DB::raw('MAX(icd9.version) as version'),
                    \DB::raw('MAX(map_icd9.system_code) as snomed_code'),
                    \DB::raw('MAX(map_icd9.system_term) as snomed_term'),
                )
                ->leftJoin('map_icd9', 'icd9.code', '=', 'map_icd9.icd9_code')
                ->when($this->search, fn($q) => $q
                    ->where('icd9.code', 'like', "%{$this->search}%")
                    ->orWhere('icd9.display', 'like', "%{$this->search}%")
                )
                ->when($this->filterVersion, fn($q) => $q->where('icd9.version', $this->filterVersion))
                ->when($this->filterStatus === 'snomed_only',  fn($q) => $q->whereNotNull('map_icd9.system_code')->whereNotIn('icd9.code', $bpjsRegistered->keys()->toArray()))
                ->when($this->filterStatus === 'bpjs_only',    fn($q) => $q->whereNull('map_icd9.system_code')->whereIn('icd9.code', $bpjsRegistered->keys()->toArray()))
                ->when($this->filterStatus === 'complete',     fn($q) => $q->whereNotNull('map_icd9.system_code')->whereIn('icd9.code', $bpjsRegistered->keys()->toArray()))
                ->when($this->filterStatus === 'empty',        fn($q) => $q->whereNull('map_icd9.system_code')->whereNotIn('icd9.code', $bpjsRegistered->keys()->toArray()))
                ->groupBy('icd9.code')
                ->orderBy('icd9.code')
                ->paginate($this->perPage);
        });

        $totalBpjs   = BpjsIcd9::count();
        $totalSnomed = Icd9Map::count();
        $total       = Icd9::distinct('code')->count('code');
...
<div>
    <x-ui.page-header title="ICD-9CM — Mapping & UUID" subtitle="Kelola mapping SNOMED CT dan UUID BPJS untuk kode tindakan ICD-9CM dalam satu tampilan">
        <x-slot name="actions">
            <flux:dropdown>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <x-organisms.stat-card title="Total ICD-9CM" :value="number_format($total)" icon="hashtag" color="zinc" />
    ...
    <x-organisms.data-panel :padding="false">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-48">
                    <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                        placeholder="Cari kode atau nama ICD-9CM..." clearable />
                </div>
                @if (count($versions) > 0)
        ...
        <x-organisms.table>
            <x-slot:headings>
                ...
                <x-atoms.table-heading>Deskripsi ICD-9CM</x-atoms.table-heading>
            ...
            @forelse ($items as $item)
                ...
                <x-molecules.table-row wire:key="icd9-{{ $item->code }}">
                ...
                <x-molecules.table-row>
                    <x-atoms.table-cell colspan="5" align="center" class="py-16">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-sm font-semibold text-zinc-500 dark:text-primary-dark-400">Tidak ada data ICD-9CM ditemukan</p>
                        </div>
                    </x-atoms.table-cell>
                </x-molecules.table-row>
            @endforelse
        ...
        <div class="space-y-4">
            <div>
                ...
                <flux:text class="mt-0.5">
                    Untuk ICD-9CM: <span class="font-semibold text-zinc-700 dark:text-primary-dark-200">{{ $selectedName }}</span>
                    <span class="font-mono text-xs text-zinc-400 ml-1">({{ $selectedCode }})</span>
                </flux:text>
            </div>
            <livewire:components.snomed-search defaultTag="procedure" :initialSearch="$snomedInitialSearch" :key="'snomed-icd9-' . ($selectedCode ?? '')" />
    ...
    <x-organisms.modal wire:model="showSyncModal" title="Sync Semua UUID BPJS ICD-9CM" maxWidth="md">
        <div class="space-y-5">
            <div class="flex items-center gap-4">
                ...
                <p class="text-sm text-zinc-500 dark:text-primary-dark-400">
                    {{ number_format($unsyncedBpjs) }} kode ICD-9CM belum memiliki UUID BPJS
                </p>
            </div>
    ...
    <x-organisms.modal wire:model="showBpjsDetailModal" title="Detail UUID BPJS ICD-9CM" maxWidth="md">
        @if ($selectedBpjsIcd)
            <dl class="space-y-4">
                <div>
                    <dt class="text-[10px] uppercase font-bold text-zinc-400 dark:text-primary-dark-500 mb-1">Kode ICD-9CM</dt>
