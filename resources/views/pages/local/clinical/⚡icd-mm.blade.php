...
new #[Layout('layouts::app')] #[Title('Mapping ICD-MM → SNOMED CT')] class extends Component {
    ...
    {
        $items = IcdMm::query()
            ->select('icd_mm.*', 'map_icd_mm.system_code as snomed_code')
            ->leftJoin('map_icd_mm', function ($join) {
                $join->on('icd_mm.code', '=', 'map_icd_mm.code')->on('icd_mm.version', '=', 'map_icd_mm.version');
            })
            ->when($this->search, fn($q) => $q->where('icd_mm.code', 'like', "%{$this->search}%")->orWhere('icd_mm.display', 'like', "%{$this->search}%"))
            ->when($this->filterVersion, fn($q) => $q->where('icd_mm.version', $this->filterVersion))
            ->orderBy('icd_mm.version')
            ->orderBy('icd_mm.code')
    ...
    public function with(): array
    {
        ...
        $kunciData = 'icd_mm.data.' . md5("{$this->search}|{$this->filterVersion}|{$this->perPage}|{$halaman}");

        $items = Cache::remember(
            $kunciData,
            300,
            fn() => IcdMm::query()
                ->select('icd_mm.*', 'map_icd_mm.system_code as snomed_code', 'map_icd_mm.system_term  as snomed_term')
                ->leftJoin('map_icd_mm', function ($join) {
                    $join->on('icd_mm.code', '=', 'map_icd_mm.code')->on('icd_mm.version', '=', 'map_icd_mm.version');
                })
                ->when($this->search, fn($q) => $q->where('icd_mm.code', 'like', "%{$this->search}%")->orWhere('icd_mm.display', 'like', "%{$this->search}%"))
    ...
    public function openModal(string $code, string $name, string $version): void
    {
        ...
        $this->snomedInitialSearch = IcdMmMap::where('code', $code)->where('version', $version)->value('system_term') ?? '';
    ...
    {
        IcdMmMap::where('code', $this->deleteCode)->where('version', $this->deleteVersion)->delete();
        Cache::flush();
...
<div>
    <x-ui.page-header title="Mapping ICD-MM → SNOMED CT"
        subtitle="Hubungkan kode kematian maternal ICD-MM dengan konsep SNOMED CT lokal" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex-1 min-w-40">
                    <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                        placeholder="Cari kode atau deskripsi ICD-MM..." clearable />
                </div>
                @if (count($versions) > 0)
    ...
    <x-organisms.modal wire:model="showSnomedSearchModal" maxWidth="4xl" title="Pilih Kode SNOMED CT">
        <div class="space-y-4">
            ...
            <livewire:components.snomed-search defaultTag="disorder" :initialSearch="$snomedInitialSearch" :key="'snomed-icd-mm-' . ($selectedCode ?? '') . '-' . ($selectedVersion ?? '')" />
