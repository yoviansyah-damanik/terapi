...
/** Halaman mapping tenaga penunjang medis ke SNOMED CT + UUID BPJS + IHS Satu Sehat. */
new #[Layout('layouts::app')] #[Title('Mapping Penunjang Medis → SNOMED CT')] class extends Component {
    use WithPagination;
    ...
    {
        SyncBpjsPractitionersJob::dispatch('Penunjang Medis');
        $this->showSyncModal = false;
        $this->toastSuccess('Sync BPJS Tenaga Penunjang Medis telah dijadwalkan di queue.', 'Dijadwalkan');
    }
    ...
    public function syncAllSatuSehat(): void
    {
        ...
        $this->toastSuccess('Sync Satu Sehat Tenaga Penunjang Medis telah dijadwalkan di queue.', 'Dijadwalkan');
    }
    ...
    public function with(): array
    {
        ...
        $query = Pegawai::where('stts_aktif', 'AKTIF')->where('bidang', 'Penunjang Medis')->when($this->search, fn($q) => $q->where('nama', 'like', "%{$this->search}%")->orWhere('id', 'like', "%{$this->search}%"))->orderBy('nama');
...
<div>
    <x-ui.page-header title="Mapping Penunjang Medis → SNOMED CT"
        subtitle="Hubungkan tenaga penunjang medis dengan kode okupasi SNOMED CT, UUID BPJS, dan IHS Satu Sehat">
        <x-slot:actions>
    ...
    <x-organisms.modal wire:model="showSyncSsModal" maxWidth="sm" title="Sync Semua ke Satu Sehat">
        <div class="space-y-6">
            ...
            <flux:text class="text-sm text-zinc-600 dark:text-primary-dark-300">
                Akan mencari dan menyimpan IHS Number untuk semua tenaga penunjang medis yang belum terdaftar di Satu
                Sehat.
            </flux:text>
    ...
    {{-- Modal: Sync BPJS --}}
    <x-organisms.modal wire:model="showSyncModal" maxWidth="sm" title="Sync UUID BPJS Tenaga Penunjang Medis">
        <div class="space-y-6">
            <div class="flex items-center gap-4">
                <div
