...
/** Halaman mapping tenaga keperawatan & kebidanan ke SNOMED CT + UUID BPJS + IHS Satu Sehat. */
new #[Layout('layouts::app')] #[Title('Mapping Keperawatan → SNOMED CT')] class extends Component {
    use WithPagination;
    ...
    {
        SyncBpjsPractitionersJob::dispatch('Keperawatan');
        $this->showSyncModal = false;
        $this->toastSuccess('Sync BPJS Keperawatan & Kebidanan telah dijadwalkan di queue.', 'Dijadwalkan');
    }
    ...
    public function syncAllSatuSehat(): void
    {
        ...
        $this->toastSuccess('Sync Satu Sehat Keperawatan & Kebidanan telah dijadwalkan di queue.', 'Dijadwalkan');
    }
...
<div>
    <x-ui.page-header title="Mapping Keperawatan & Kebidanan → SNOMED CT"
        subtitle="Hubungkan tenaga keperawatan dan kebidanan dengan kode okupasi SNOMED CT, UUID BPJS, dan IHS Satu Sehat">
        <x-slot:actions>
    ...
    <x-organisms.modal wire:model="showSyncSsModal" maxWidth="sm" title="Sync Semua ke Satu Sehat">
        <div class="space-y-6">
            ...
            <flux:text class="text-sm text-zinc-600 dark:text-primary-dark-300">
                Akan mencari dan menyimpan IHS Number untuk semua tenaga keperawatan & kebidanan yang belum terdaftar di
                Satu Sehat.
            </flux:text>
    ...
    {{-- Modal: Sync BPJS --}}
    <x-organisms.modal wire:model="showSyncModal" maxWidth="sm" title="Sync UUID BPJS Keperawatan & Kebidanan">
        <div class="space-y-6">
            <div class="flex items-center gap-4">
                <div
